...
const SITE = {
  ...
  tagline: process.env.SITE_TAGLINE || "Mahjong, hikes, film nights, cabins and parties — hosted in San Francisco.",
};
...
function nextCard(cat, ev) {
  const soon = countdown(ev.start_date);
  const p = parts(ev.start_date, ev.timezone);
  return `
    <article class="card" data-category="${cat.slug}">
      <a class="card__media" href="${esc(ev.url)}" target="_blank" rel="noopener">
        ${ev.image_url ? `<img src="${esc(ev.image_url)}" alt="" loading="lazy">` : `<span class="card__fallback">${cat.emoji}</span>`}
...
function archiveRow(ev) {
  ...
  return `
    <a class="past" href="${esc(ev.url)}" target="_blank" rel="noopener" data-category="${esc(ev.category)}">
      <div class="past__thumb">
        ${ev.image_url ? `<img src="${esc(ev.image_url)}" alt="" loading="lazy">` : `<span>${cat.emoji}</span>`}
      </div>
...
export function homePage({ next, past, thanks }) {
  const upcomingCount = Object.keys(next).length;
  const scheduled = CATEGORIES.filter((c) => next[c.slug]);
  const pending = CATEGORIES.filter((c) => !next[c.slug]);
  const years = [...new Set(past.map((e) => (e.start_date ? new Date(e.start_date).getUTCFullYear() : "—")))];
...
<main>
  <section id="up-next" class="section">
    <div class="section__head">
      <h2>Up next</h2>
      <p>${upcomingCount ? "The soonest one in each category. RSVPs happen on Partiful." : "Nothing scheduled right now — leave your info below and you'll hear about the next one."}</p>
    </div>
    ${scheduled.length ? `<div class="grid${scheduled.length === 1 ? " grid--solo" : ""}">${scheduled.map((c) => nextCard(c, next[c.slug])).join("\n")}</div>` : ""}
    ${pending.length ? `
    <div class="pendingwrap">
      <p class="pendingwrap__head">${scheduled.length ? "Not on the calendar yet" : "Nothing scheduled yet"} — say the word and I'll plan one</p>
  ...
  <section id="archive" class="section section--alt">
    <div class="section__head">
      <h2>The archive</h2>
      <p>${past.length ? `${past.length} ${past.length === 1 ? "night" : "nights"} so far${years.length ? `, ${years[years.length - 1]}–${years[0]}` : ""}.` : "Past events will collect here once they've happened."}</p>
    </div>
    ${past.length ? `
  ...
  <section class="admin__panel">
    ...
    <div class="tblwrap"><table class="tbl">
      ...
      <tbody>
        ...
        <tr>
          <td class="nowrap">${e.start_date ? esc(fmtLong(e.start_date, e.timezone)) : "—"}<br><small>${e.start_date ? esc(new Date(e.start_date).getUTCFullYear()) : ""}</small></td>
          <td><a href="${esc(e.url)}" target="_blank" rel="noopener">${esc(e.title)}</a><br><small>${esc(e.location || "")}</small></td>
          <td>
