  ...
  return `
    <form class="cform" method="post" action="${action}">
      <div class="cform__row">
        ...
        <label><span class="lbl">Email</span>
          <input type="email" name="email" maxlength="120" value="${esc(value.email || "")}" placeholder="them@example.com">
        </label>
        <label><span class="lbl">Phone</span>
      ...
      <div class="cform__actions">
        ...
        ${id ? `<button class="linkbtn linkbtn--bad" type="submit" formaction="/admin/signups/${id}/delete"
                  onclick="return confirm('Delete this contact?')">Delete</button>` : ""}
      </div>
    </form>`;
}

export function adminPage({ events, signups, flash, error, editing = null, addOpen = false }) {
...
<div class="admin">
  ...
  <section class="admin__panel" id="contacts">
    <h2>Contacts <span class="count">${signups.length}</span>
    ...
    <details class="adder"${addOpen ? " open" : ""}>
      ...
      ${contactForm({ action: "/admin/signups", submit: "Add contact" })}
    </details>
      ...
      <li class="contact">
        <details${String(editing) === String(c.id) ? " open" : ""}>
          <summary>
            ...
            <span class="contact__contact">${[c.email, c.phone].filter(Boolean).map(esc).join(" · ") || "<em>no contact details</em>"}</span>
          ...
          ${contactForm({ action: `/admin/signups/${c.id}`, submit: "Save changes", value: c, picked, id: c.id })}
        </details>
      </li>`;
      }).join("")}
    </ul>` : `<p class="empty">No contacts yet. People who fill in the form on the site land here, or add one by hand above.</p>`}
    <p class="hint">Only visible here, behind the admin password. Every change also rewrites <code>data/signups.csv</code> on this machine.</p>
  </section>
  <section class="admin__panel">
