...
<div class="admin">
  ...
  ${counts.length ? `
  <div class="tally">
    <span class="tally__total">${signups.length} ${signups.length === 1 ? "person" : "people"}</span>
    ${counts.map((c) => `<span class="tag" data-category="${c.slug}">${c.emoji} ${esc(c.label)} <b>${c.n}</b></span>`).join("")}
  </div>` : ""}
...
export function adminPage({ events, flash, error }) {
