...
// A hung request must not stall the background sync loop behind it.
...
async function canonicalUrl(input) {
  ...
  const res = await fetch(raw, { redirect: "follow", headers: { "user-agent": UA }, signal: AbortSignal.timeout(FETCH_TIMEOUT_MS) });
