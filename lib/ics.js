...
function vevent(ev, now) {
  ...
  return [
    ...
    // A moved date should update the event in place rather than add a second one.
    `SEQUENCE:${Math.floor(new Date(ev.synced_at || ev.added_at || now).getTime() / 60000) % 100000}`,
    "END:VEVENT",
  ].filter(Boolean);
}
