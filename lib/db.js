...
export function addManualEvent({ id, title, description, category, location, startDate = null }) {
  const now = new Date().toISOString();
  db.query(`
    INSERT INTO events (id, url, category, title, description, start_date, timezone, location, hidden, added_at, synced_at)
    VALUES ($id, '', $category, $title, $description, $start_date, 'America/Los_Angeles', $location, 0, $now, $now)
...
export function upsertEvent(ev, category) {
  ...
  db.query(`
    INSERT INTO events (id, url, category, title, description, start_date, end_date,
                        timezone, image_url, image_thumb, location, going_count, hidden, added_at, synced_at)
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      image_url = $image_url, image_thumb = $image_thumb, location = $location, going_count = $going_count,
      synced_at = $synced_at
  `).run({
    $id: ev.id,
    $url: ev.url,
