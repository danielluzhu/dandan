...
export function updateSignup(id, { name, email, phone, categories, note }) {
  db.query(`
    UPDATE signups SET name = ?, email = ?, phone = ?, categories = ?, note = ?
    WHERE id = ?
  `).run(name, email || null, phone || null, categories.join(","), note || null, Number(id));
}
